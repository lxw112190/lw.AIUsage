...
<template>
  <section class="page">
    ...
    <article class="panel filter-panel">
      ...
      <div class="filter-field"><label>{{ t("filter.agent") }}</label><select v-model="store.sourceFilter"><option value="">{{ t("filter.allAgents") }}</option><option value="codex">Codex</option><option value="claude">Claude Code</option></select></div>
      <div class="filter-field"><label>{{ t("filter.model") }}</label><select v-model="store.modelFilter"><option value="">{{ t("filter.allModels") }}</option><option v-for="model in store.modelOptions" :key="model" :value="model">{{ model }}</option></select></div>
      <div class="filter-field"><label>{{ t("filter.project") }}</label><select v-model="store.projectFilter"><option value="">{{ t("filter.allProjects") }}</option><option v-for="project in store.projectOptions" :key="project.key" :value="project.key">{{ project.name }}</option></select></div>
      <button class="clear-button" @click="store.clearFilters">{{ t("filter.clear") }}</button>
    </article>
    <article class="panel table-panel" :class="{ 'is-loading': store.loading }">
      ...
      <div class="table-footer">
        <span>{{ store.total }} {{ t("usage.recordsCount") }}</span>
        <label>{{ t("usage.pageSize") }} <select :value="store.pageSize" @change="store.setPageSize(Number(($event.target as HTMLSelectElement).value))"><option v-for="size in store.pageSizeOptions" :key="size" :value="size">{{ size }}</option></select></label>
        <div class="pagination"><button :disabled="store.page <= 1" @click="store.setPage(store.page - 1)">‹</button><template v-for="(item, index) in pages" :key="`${item}-${index}`"><span v-if="item === '…'" class="page-ellipsis">…</span><button v-else :class="{ active: item === store.page }" @click="store.setPage(item)">{{ item }}</button></template><button :disabled="store.page >= store.totalPages" @click="store.setPage(store.page + 1)">›</button></div>
      </div>
    </article>
  </section>
</template>
