...
<template>
  <section class="page">
    ...
    <article class="panel filter-panel">
      ...
      <div class="filter-field">
        <label>{{ t("filter.agent") }}</label
        ><select v-model="store.sourceFilter">
      ...
      <div class="filter-field">
        <label>{{ t("filter.model") }}</label
        ><select v-model="store.modelFilter">
      ...
      <button class="clear-button" @click="store.clearFilters">
        {{ t("filter.clear") }}
      </button>
    </article>
    <article class="panel table-panel">
      <table>
        ...
        <tbody>
          <tr v-for="record in store.filteredRecords" :key="record.id">
            <td>{{ dateTime(record.timestamp) }}</td>
            <td>
              <span class="source-badge">{{ record.source }}</span>
            </td>
            <td>{{ record.model }}</td>
            <td>{{ projectDisplayName(record.projectKey) }}</td>
            <td>{{ format(record.usage.inputTokens) }}</td>
            <td>{{ format(record.usage.cachedInputTokens) }}</td>
            <td>{{ format(record.usage.outputTokens) }}</td>
            <td>
              <strong>{{ format(totalTokens(record.usage)) }}</strong>
            </td>
          </tr>
          <tr v-if="!store.filteredRecords.length">
            <td colspan="8" class="empty-cell">{{ t("usage.empty") }}</td>
          </tr>
        </tbody>
      </table>
    </article>
