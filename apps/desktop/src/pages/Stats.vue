...
<template>
  <section class="page">
    ...
    <div class="stats-columns">
      ...
      <article class="panel"><div class="panel-heading"><div><h2>{{ t("stats.projects") }}</h2><p>{{ t("stats.projectDistribution") }}</p></div></div><div class="stats-table-wrap"><table class="compact-table project-stats-table"><thead><tr><th>{{ t("filter.project") }}</th><th>{{ t("stats.records") }}</th><th>{{ t("stats.tokens") }}</th><th>{{ t("stats.cost") }}</th></tr></thead><tbody><tr v-for="item in store.data.byProject" :key="item.key"><td class="project-name-cell"><strong>{{ projectDisplayName(item.key) }}</strong></td><td>{{ item.recordCount }}</td><td>{{ format(item.totalTokens) }}</td><td>{{ money(item.estimatedCostUsd) }}</td></tr><tr v-if="!store.data.byProject.length"><td colspan="4" class="empty-cell">{{ t("stats.noProjectData") }}</td></tr></tbody></table></div></article>
    </div>
  </section>
</template>
