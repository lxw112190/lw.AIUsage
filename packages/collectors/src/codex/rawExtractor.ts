...
export async function extractCodexFile(platform: RuntimePlatform, entry: FileEntry, snapshotSize = entry.size, collectorLogicalId?: string): Promise<CodexExtractedFile> {
  ...
  const result: CodexExtractedFile = { entry, snapshotSize, peekLogicalId: collectorLogicalId ?? peek.sessionId, peekForkedFromId: peek.forkedFromId, events: [], parseErrors: [] };
  ...
  const consumeText = (text: string): void => { const parsed = parseJsonl<UnknownCodexEvent>(text, pendingText); pendingText = parsed.pendingText; result.parseErrors.push(...parsed.errors); for (const event of parsed.values) consume(event); };
