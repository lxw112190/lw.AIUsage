...
const eventTypeOf = (event: UnknownCodexEvent): string | undefined => codexEventTypeInfo(event).semanticType;
...
export async function extractCodexFile(platform: RuntimePlatform, entry: FileEntry, snapshotSize = entry.size, collectorLogicalId?: string): Promise<CodexExtractedFile> {
  ...
  const consume = (event: UnknownCodexEvent): void => { const taxonomy: CodexEventTypeInfo = codexEventTypeInfo(event); const source = sourceOf(event, taxonomy); const type = taxonomy.semanticType; const modelBefore = currentModel; const model = modelOf(event, currentModel); const session = sessionOf(event, currentSession); const explicitTimestamp = timestampOf(event); const forkedFromId = forkOf(event); if (model) currentModel = model; if (session) currentSession = session; if (type === "session_meta") { result.finalSessionId ??= session; result.peekForkedFromId ??= forkedFromId; } result.events.push({ raw: event, eventIndex, eventType: type, explicitTimestamp, previousEventType, modelBefore, resolvedModel: model, resolvedSessionId: session, forkedFromId: forkedFromId ?? result.peekForkedFromId, total: source.total, last: source.last, flat: source.flat, source: source.source, outerType: taxonomy.outerType, payloadType: taxonomy.payloadType, messageType: taxonomy.messageType, semanticType: taxonomy.semanticType, isTokenCount: taxonomy.isTokenCount }); eventIndex += 1; previousEventType = type ?? previousEventType; };
